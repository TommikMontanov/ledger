...
async def start_ob(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Пришлите файлы банковской выписки (.xlsx).\nМожно отправить несколько штук. После загрузки нажмите '✅ Готово'.",
...
async def act_db_start(message: Message, state: FSMContext):
    await message.answer(
        "Пришлите файл Оборотки (.xlsx) для загрузки в Базу Актов.\nПредыдущая база будет удалена и заменена новой.",
...
async def act_generate_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Отправьте счета-фактуры в формате **.csv**.\nМожно выделять и отправлять сразу пачку.\nКак закончите — нажмите '✅ Готово'.",
        reply_markup=files_kb, parse_mode="Markdown")
